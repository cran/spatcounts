...
void gammaindbisection(int* nb, double* beta1, int* J, double* ga,  int* ny, double* mu,  double* phi, double* psi, double* invsigmasq, double* x, double* y, double* ex, int* acceptg, int* nmat, int* maxindex, double* gamma_r)
{
...
while(fabs(gaup - galo) > 0.02*fabs(gaup)) {
   if (dlm < 0){
      dlup = dlm;
      gaup = gam1;
      gam1 = (galo + gaup)/2;
